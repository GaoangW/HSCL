...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    normal_vec = model.normal_vec.detach()
    scores_id = get_scores(P, feats_id, ood_score, normal_vec).numpy()
    all_feat = feats_id['simclr'][:,0,:].detach().numpy()
    ...
    for ood, feats in feats_ood.items():
        scores_ood[ood] = get_scores(P, feats, ood_score, normal_vec).numpy()
        auroc_dict[ood][ood_score] = get_auroc(scores_id, scores_ood[ood])

        tmp_feat = feats['simclr'][:,0,:].detach().numpy()
        all_feat = np.concatenate([all_feat, tmp_feat], axis=0)
...
def get_scores(P, feats_dict, ood_score, normal_vec):
    ...
    for f_sim, f_shi in zip(feats_sim, feats_shi):
        ...
        for shi in range(P.K_shift):
            ...
            score += torch.mean(torch.max(torch.mm(f_sim[shi], normal_vec), dim=1)[0]).item() * P.weight_sim[shi]
...
def _get_features(P, model, loader, interp=False, imagenet=False, simclr_aug=None,
    ...
    vis_feat = []
    vis_label = []
    for i, (x, tmp_label) in enumerate(loader):
        vis_label.append(tmp_label)
